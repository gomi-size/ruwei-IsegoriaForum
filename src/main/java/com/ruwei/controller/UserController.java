...
public class UserController {
    ...
    /**
     * 修改密码（已登录场景，只能改本人的；需当前绑定邮箱验证码）。
     *
     * <p><b>契约变更（前端需同步调整）</b>：入参由 query 参数 {@code ?id=&password=}
     * 改为 JSON 请求体 {@link EditPasswordDTO}，新增 {@code checkPassword} 二次确认
     * 与 {@code code} 邮箱验证码（需先调 {@code POST /user/email/code} 传
     * {@code scene=changePwd} 向当前绑定邮箱发码，邮箱由服务端从登录态读取）。</p>
     *
     * <p>{@code id} 一律传<b>内部主键 id</b>（即登录接口返回的用户对象里的 {@code id} 字段），
